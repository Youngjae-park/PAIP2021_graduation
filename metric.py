...
def compute_f1_score(
    ...
    # [0,1,2,3, ...] , [minimum value of each row]
    ...
    return f1_score,\
           (gt_count, pred_count),\
           [list(matched_gt_labels),
            list(unmatched_pred_labels),
            list(set(range(gt_count)) - set(matched_gt_labels)),
            list(set(matched_pred_labels)),
            precision,
            recall]
...
def worker(gt, pred, offset, table, use_tqdm=False):
    gt_labels, gt_label_map = gt
    pred_labels, pred_label_map = pred

    if offset==0 and use_tqdm:
        range_fn = tqdm.tqdm(range(max(offset, 1), gt_labels))
    else:
        range_fn = range(max(offset, 1), gt_labels)
    for i in range_fn:  # Exclude background label
        ...
        for j in range(1, pred_labels):
            ...
            table[i - 1, j - 1] = dist_score_ij


def threading_compute_dist_f1_score(
        pred: np.ndarray,
        gt: np.ndarray,
        num_thread: int=8,
) -> float:
    ...
    # print(gt_labels, pred_labels, gt_label_map.shape, pred_label_map.shape)

    split_size = gt_labels//num_thread+1
    th = []
    for i in range(num_thread):
        gt = (min(gt_labels, (i+1)*split_size), gt_label_map)
        th.append(threading.Thread(target=worker, args=(gt, (pred_labels, pred_label_map), i*split_size, score_table)))
        th[-1].start()
    for t in th:
        t.join()
    # print(score_table.shape, score_table, gt_label_map.shape, pred_label_map.shape)
    # print(np.unique(gt_label_map, return_counts=True))
    # print(np.unique(pred_label_map, return_counts=True))
    f1_score, counts, others = compute_f1_score(score_table, gt_labels, pred_labels)

    return f1_score, counts, others, [gt_label_map, pred_label_map]
...
def compute_dist_f1_score(
    ...
    for i in tqdm.tqdm(range(1, gt_labels)):  # Exclude background label
        gt_i = (gt_label_map == i).astype(np.uint8)
        gt_i_bbox, gt_start_x, gt_end_x, gt_start_y, gt_end_y = \
            bbox_map(gt_i)
